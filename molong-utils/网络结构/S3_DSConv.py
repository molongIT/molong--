...
class DSConv(nn.Module):
    ...
    def __init__(self, in_ch, out_ch, kernel_size, extend_scope, morph,
        ...
        :param extend_scope: 扩展范围（此方法默认为1）
        :param morph: 卷积核的形态，主要分为沿 x 轴（0）和 y 轴（1）两种（详细信息见论文）
        :param if_offset: 是否需要变形，如果为 False，则为标准卷积核
        :param device: 设置为 GPU
        """
        super(DSConv, self).__init__()
        # 使用 <offset_conv> 学习可变形的偏移量
        self.offset_conv = nn.Conv2d(in_ch, 2 * kernel_size, 3, padding=1)
        self.bn = nn.BatchNorm2d(2 * kernel_size)
        self.kernel_size = kernel_size

        # 两种类型的 DSConv（沿 x 轴和 y 轴）
    ...
    def forward(self, f):
        ...
        # 我们需要在 -1 到 1 的范围内进行变形，以模拟蛇的摆动
        # 使用 torch.tanh 将偏移量范围限制在 -1 到 1 之间，以模拟蛇的摆动。
        offset = torch.tanh(offset)
        input_shape = f.shape
        #使用 DSC 实例的 deform_conv 方法进行可变形卷积操作。
        dsc = DSC(input_shape, self.kernel_size, self.extend_scope, self.morph,
                  self.device)
        deformed_feature = dsc.deform_conv(f, offset, self.if_offset)
        # 根据卷积核的形态（x 轴或 y 轴）选择不同的卷积层进行卷积操作，归一化并激活，最后返回结果。
        if self.morph == 0:
...
# 核心代码，这个类负责生成坐标图和执行双线性插值，以实现可变形卷积。
class DSC(object):
    ...
    def __init__(self, input_shape, kernel_size, extend_scope, morph, device):
        ...
        self.morph = morph
        self.device = device
        self.extend_scope = extend_scope   # 偏移范围 (-1 ~ 1) * extend_scope

        # 定义特征图的形状
        """
        B: 批量大小  C: 通道数  W: 宽度  H: 高度
        """
        self.num_batch = input_shape[0]
        self.num_channels = input_shape[1]

    """
    输入: offset [B,2*K,W,H]  K: 卷积核大小 (2*K: 2D 图像，变形包含 <x_offset> 和 <y_offset>)
    output_x: [B,1,W,K*H]   坐标图
    output_y: [B,1,K*W,H]   坐标图
    """
     
    # 定义 _coordinate_map_3D 方法，用于生成 3D 坐标图。
    def _coordinate_map_3D(self, offset, if_offset):
        ...
        if self.morph == 0:
            ...
            初始化卷积核并将其展平
                y: 只需要 0
                x: -num_points//2 ~ num_points//2 （由卷积核大小决定）
                !!! 相关的 PPT 将在稍后提交，PPT 将包含每一步的完整变化
            """
            ...
            return y_new, x_new
    """
    输入: 输入特征图 [N,C,D,W,H]；坐标图 [N,K*D,K*W,K*H]
    输出: [N,1,K*D,K*W,K*H]  变形后的特征图
    """
    ...
    """ 
    •	定义 deform_conv 方法，用于进行变形卷积。
	•	使用 _coordinate_map_3D 方法生成 y 和 x 坐标。
	•	使用 _bilinear_interpolate_3D 方法在特征图上进行双线性插值，得到变形后的特征图。
	•	返回变形后的特征图。
    """
